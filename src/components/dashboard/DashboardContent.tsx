...
const FOLDER_LINKS: Record<string, string> = {
  "Share Lagat": "https://docs.google.com/spreadsheets/d/1T2xCfx0UDpJ_NDfvtK3HGCmHUBGIU8cIHeyHVX89GdM/edit?gid=1866054730#gid=1866054730",
  "Name Change": "https://docs.google.com/spreadsheets/d/1Dar7IKb53ZroJWCqpQfBtsf9jEPXahMpKrVQvX6I29o/edit?gid=812097579#gid=812097579",
  "Company Registration Documents": "https://docs.google.com/spreadsheets/d/14p0wjJoovZeUtwDwKqp183uPjTzdpOMHDKXy8tmKZJ4/edit?gid=1046074706#gid=1046074706",
  "Amendment Of Objectives": "https://docs.google.com/spreadsheets/d/1IUfiWhWxzTlg0uyt7s8ZMwV0smLGcVkgPwv8pPFTXdo/edit?gid=285174755#gid=285174755",
  "Address Change": "https://docs.google.com/spreadsheets/d/1HyfBNTRCAbYmzjEdbznN-AOnj6Tgwsnlr9JkKDS57ms/edit?gid=577051600#gid=577051600",
};

const OUTPUT_LINKS: Record<string, string> = {
  "Share Lagat": "https://drive.google.com/drive/folders/1K02AGpV7G91YCU4Q-Sd2yASC9nHwUoy6",
  "Name Change": "https://drive.google.com/drive/folders/1uOuFjS5cAxDPejqEnry2IlDhPU-TzPw-",
  "Company Registration Documents": "https://drive.google.com/drive/folders/18lIdV79JJRxzX0Zi1j3aHobS6tDSPi9o",
  "Amendment Of Objectives": "https://drive.google.com/drive/folders/1AZG-9uDvWMRtEqMAjaTvhZj2aji5fVkG",
  "Address Change": "https://drive.google.com/drive/folders/1Rn98ay9wslTv-HTU121m7xULyoWpmmcP",
};
...
export function DashboardContent({ userEmail }: DashboardContentProps) {
  ...
  const breadcrumbs = [
    { name: activeView === "dashboard" ? "Dashboard" : activeView.charAt(0).toUpperCase() + activeView.slice(1), 
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message?.includes('P1001') || err.message?.includes('database server')) {
        setCreationMessage("Database is temporarily unreachable. Please try again soon.");
      } else {
        setCreationMessage(err instanceof Error ? err.message : "Failed to create user");
      }
    }
  };
        ...
        {excelLink ? (
          <a href={excelLink} target="_blank" rel="noopener noreferrer" 
            ...
            <div className="flex-1">
              <h4 className="m-0 text-primary font-bold text-lg">{currentFolder} Sheet</h4>
              <p className="m-1 text-secondary text-sm">Google Spreadsheet • Click to open in new tab</p>
            </div>
          ...
          <div className="p-12 text-center bg-white border border-dashed border-border rounded-DEFAULT">
            ...
            <p className="text-secondary text-sm">There is no specific spreadsheet linked for "{currentFolder}" yet.</p>
          </div>
        )}
      </div>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white border border-border rounded-DEFAULT p-8 flex flex-col items-center h-fit">
          ...
          <div className="w-full mt-8 pt-6 border-t border-border space-y-4">
            <div>
              <label className="block text-[10px] font-bold text-secondary uppercase tracking-widest mb-1">Email</label>
              <div className="text-sm font-medium text-primary break-all">{userEmail}</div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white border border-border rounded-DEFAULT p-8">
            ...
            {creationMessage && (
              <div className={`p-4 mb-6 text-sm font-medium rounded-DEFAULT border ${
                creationStatus === "success" ? "bg-green-50 border-green-100 text-green-700" : "bg-red-50 border-red-100 text-red-700"
              }`}>
                {creationMessage}
              </div>
            )}
            ...
            <form onSubmit={handleCreateUser} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="block text-sm font-bold text-primary">User Email Address</label>
                <input 
                  type="email" 
                  value={newUserEmail}
              ...
              <div className="space-y-2">
                <label className="block text-sm font-bold text-primary">Temporary Password</label>
                <input 
                  type="password" 
          ...
          <div className="bg-white border border-border rounded-DEFAULT p-8">
            <h3 className="text-xl font-bold text-primary mb-4 pb-3 border-b border-border">System Info</h3>
            <div className="space-y-4">
              <div className="flex justify-between items-center py-2">
                <span className="text-sm font-medium text-secondary">Account Status</span>
                <span className="px-2 py-1 bg-green-100 text-green-700 text-[10px] font-bold rounded uppercase">Active</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-sm font-medium text-secondary">Last Login</span>
    ...
    if (!currentFolder) {
      return (
        <div className="p-8 max-w-[1400px] mx-auto animate-[fadeIn_0.4s_ease-out]">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-primary mb-2">Output Documents</h2>
            <p className="text-secondary text-sm">Select a category to view generated output links</p>
          </div>
          <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-6">
            {FOLDERS.map((folder) => (
              <FolderCard key={folder} name={folder} onClick={() => handleFolderClick(folder)} />
      ...
      <div className="p-8 max-w-[1400px] mx-auto animate-[fadeIn_0.4s_ease-out]">
        ...
        <div className="max-w-3xl mt-6">
          <div className="bg-white border border-border rounded-DEFAULT p-12 text-center flex flex-col items-center gap-6 shadow-sm group">
            ...
            <h2 className="text-3xl font-extrabold text-primary m-0 tracking-tight">{currentFolder} Output</h2>
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3 pr-6 border-r border-border">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-bold text-primary leading-tight">{userEmail.split('@')[0]}</span>
              <span className="text-xs text-secondary font-medium uppercase tracking-wider">Administrator</span>
            </div>
          </div>
          <form action="/api/auth/logout" method="POST">
            <button type="submit" className="flex items-center justify-center w-10 h-10 rounded-full border border-border text-secondary transition-all hover:bg-red-50 hover:text-red-500 hover:border-red-200" title="Logout">
